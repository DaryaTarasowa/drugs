...
function initiate_parallel() { //for reset
		d3.csv("drug-2.csv", function(raw_data) {
	  // Convert quantitative scales to floats
	  data = raw_data.map(function(d) {
	    for (var k in d) {
...
function data_table(sample) {
  ...
  table
    .append("span")
      .text(function(d) { return d.GENERIC_NAME; })
}
...
function highlight(d) {
  ...
  d3.selectAll(".table_row").style("opacity", function(p) { return (d.group == p) ? null : "0.3" });
...
function unhighlight() {
  d3.select("#foreground").style("opacity", null);
  d3.selectAll(".table_row").style("opacity", null);
